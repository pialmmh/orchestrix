...
class StoreTestRunner {
  ...
  async runAllTests() {
    ...
    await this.runTest('Load Infrastructure Tree', async () => {
      const infraStore = this.storeManager.getStore('infrastructure');
    ...
    await this.runTest('Store Snapshot Creation', async () => {
      const snapshot = this.storeManager.loadSnapshot('infrastructure');

      if (!snapshot) {
        // Try to wait a bit for snapshot to be created
        await new Promise(resolve => setTimeout(resolve, 2000));

        const retrySnapshot = this.storeManager.loadSnapshot('infrastructure');
        if (!retrySnapshot) {
