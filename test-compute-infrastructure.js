...
(async () => {
  ...
  try {
    ...
    // Click on Infrastructure menu item
    console.log('Clicking Infrastructure menu item...');
    const infrastructureMenuItem = await page.locator('text="Infrastructure"').first();
    if (await infrastructureMenuItem.count() > 0) {
      await infrastructureMenuItem.click();
      ...
      // Look for expand icons and expand the tree
      console.log('Looking for expandable nodes...');
      const expandIcons = await page.locator('[data-testid*="Expand"], .MuiTreeItem-iconContainer svg').all();
      ...
      // Now look for specific nodes we can add compute to
      const nodeTexts = ['Cloud Computing', 'Compute', 'TestDC-with-groups', 'Virtual', 'Physical', 'Dhaka-DC1'];
      let foundNode = false;
      ...
      if (await addComputeMenuItem.count() > 0) {
        ...
        if (await dialogTitle.count() === 0) {
          ...
          // Check if the new compute appears in the tree
          await page.waitForTimeout(2000);
          const newCompute = await page.locator('text="mustafa-pc"');
          if (await newCompute.count() > 0) {
            console.log('✓ New compute "mustafa-pc" appears in the tree!');
          }
        } else {
      ...
      console.log('Could not find Infrastructure menu item');
    }
