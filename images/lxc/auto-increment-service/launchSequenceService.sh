...
if [ $# -eq 0 ]; then
    ...
    echo "Example: $0 ./sequence-service-v.2.0.0/generated/sample.conf"
    exit 1
fi
...
echo "========================================="
echo "Sequence Service Container Launcher"
echo "========================================="

source "$CONFIG_FILE"

# Set defaults
CONTAINER_NAME="${CONTAINER_NAME:-sequence-1}"
BASE_IMAGE="${BASE_IMAGE:-sequence-service-base}"
...
start_service() {
    echo ""
    echo "Starting sequence service..."

    lxc exec "$CONTAINER_NAME" -- bash -c "
        systemctl start sequence-service.service
        sleep 2
        systemctl status sequence-service.service --no-pager
    "
...
print_summary() {
    ...
    if [ -n "$container_ip" ]; then
        ...
        echo ""
        echo "API Endpoints:"
        echo "  GET  http://${container_ip}:${SERVICE_PORT}/api/next-id/<entity>?dataType=<type>"
        echo "       Types: int, long, uuid8, uuid12, uuid16, uuid22"
        echo "  GET  http://${container_ip}:${SERVICE_PORT}/api/next-batch/<entity>?dataType=<type>&batchSize=<n>"
        echo "  GET  http://${container_ip}:${SERVICE_PORT}/api/status/<entity>"
        echo "  GET  http://${container_ip}:${SERVICE_PORT}/api/list"
        echo "  GET  http://${container_ip}:${SERVICE_PORT}/api/types"
    fi

    echo ""
    echo "Container commands:"
    echo "  View logs:    lxc exec $CONTAINER_NAME -- tail -f /var/log/sequence-service.log"
    echo "  Reset data:   lxc exec $CONTAINER_NAME -- sequence-reset"
    ...
    echo ""
    echo "CLI Reset Examples:"
    echo "  Reset all:    lxc exec $CONTAINER_NAME -- sequence-reset"
    echo "  Reset entity: lxc exec $CONTAINER_NAME -- sequence-reset -e users"
    echo "  Set counter:  lxc exec $CONTAINER_NAME -- sequence-reset -e users -v 1000"
    echo "  Change type:  lxc exec $CONTAINER_NAME -- sequence-reset -e tokens -t uuid16"
    echo ""
